...
def CompanionStandings(message: str, seperator: str) -> NewStandings:
    data: list[Standing] = []
    errors: list[str] = []
    rows = message.split("\n")
    ...
    for i in range(len(rows)):
        try:
            ...
            if losses < 0:
                raise KnownError(
                    f"Losses cannot be negative for row {i + 1}: {rows[i]}"
                )
            draws = int(record[2])
            if draws < 0:
...
def CompanionPairings(message: str) -> NewPairings:
    data: list[Pairing] = []
    errors: list[str] = []
    rows = message.split("\n")
    for i in range(0, len(rows), 6):
        ...
        try:
            if row[3].upper() != "Bye".upper():
                ...
                if p1name == "":
                    raise KnownError(
                        f"Names cannot be blank (row {i + 2}, please resubmit rows {i + 1} - {i + 6})"
                    )
                p1gw = int(row[3][0])
                p2gw = int(row[3][1])
                if p1gw > 2 or p2gw > 2 or p1gw + p2gw >= 4:
                    raise KnownError(
                        f'Game wins cannot be greater than 2 and cannot add up to 4 or more (row {i + 4}: "{rows[i + 3]}", please resubmit rows {i + 1} - {i + 6})'
                    )
                p2name = row[4]
                if p2name == "":
                    raise KnownError(
                        f"Names cannot be blank (row {i + 5}, please resubmit rows {i + 1} - {i + 6})"
                    )
                ...
                if p1name == "":
                    raise KnownError(
                        f"Names cannot be blank (row {i + 1}, please resubmit rows {i + 1} - {i + 4})"
                    )
                p1gw = 2
                p2name = "Bye"
                p2gw = 0
