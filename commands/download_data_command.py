...
class DownloadDataGroup(commands.GroupCog, name="download"):
    ...
    async def StoreDownload(
        ...
        if len(files) == 0:
            ...
            await MessageUser(self.bot, message, interaction.user.id, files)
    ...
    async def PlayerDownload(
        ...
        if len(files) == 0:
            ...
            await MessageUser(self.bot, title, interaction.user.id, files)
