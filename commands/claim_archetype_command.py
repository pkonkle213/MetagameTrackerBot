...
class ClaimArchetype(commands.GroupCog, name='claim'):
  ...
  @app_commands.command(
      name='limited',
      description=
      'Enter the deck archetype for a player and their last played limited event'
  )
  ...
  @app_commands.command(
      name='constructed',
      description=
      'Enter the deck archetype for a player and their last played constructed event'
  )
  @app_commands.guild_only()
  async def ClaimConstructed(self, interaction: Interaction, player_name: str,
    ...
    try:
      archetype_submitted, event = await ClaimResult(interaction, player_name,
                                                     archetype, date)
      if archetype_submitted is None:
        await interaction.followup.send(
            'Unable to submit the archetype. Please try again later.')
      else:
        message = f'''
        ...
        '''
        await MessageChannel(self.bot, message, settings.BOTGUILD.id,
                             settings.CLAIMCHANNEL)
        await interaction.followup.send(
            f"Thank you for submitting the archetype for {event.EventDate.strftime('%B %d')}'s event!",
            ephemeral=True)
        ...
        if followup:
          if followup[1]:
            ...
            await MessageChannel(self.bot, output, interaction.guild_id,
                                 interaction.channel_id)
          else:
            await MessageChannel(self.bot, followup[0], interaction.guild_id,
                                 interaction.channel_id)
    except ValueError:
      await interaction.followup.send(
