...
def ConvertInputToPostgresArray(input_list:list[Card]) -> str:
  card_names = [card.name.replace("'","''") for card in input_list if card.in_mainboard]
  array_string = "',\n'".join(card_names)
      ...
      deck_cards AS (
        WITH
          cards AS (
            ...
            FROM
              decklists dl
              INNER JOIN decks de ON dl.deck_id = de.id
              INNER JOIN events e ON e.id = de.event_id
              INNER JOIN selected_format sf ON sf.id = e.format_id
            WHERE
              e.event_date > CURRENT_DATE - INTERVAL '1 year'
          )
