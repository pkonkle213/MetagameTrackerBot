...
async def AddDeck(player_name: str, event_id: int) -> int:
    async with (
        ...
        await cur.execute(command, [event_id, player_name])
        deck = await cur.fetchone()
        if deck is None:
            raise KnownError("Unable to add new deck")
...
async def AddCards(deck_id: int, cards: list[Card]) -> int:
    data_to_insert = [
        (deck_id, card.quantity, card.card_name, card.is_mainboard) for card in cards
    ]
...
def ConvertInputToPostgresArray(input_list: list[Card]) -> str:
    card_names = [
        card.card_name.replace("'", "''") for card in input_list if card.is_mainboard
    ]
    array_string = "',\n'".join(card_names)
    return f"'{array_string}'"


async def SelectArchetype(cards: list[Card], format: Format) -> str:
    ...
    async with (
        ...
        await cur.execute(command, [format.id, maindeck])
        archetype = await cur.fetchone()
