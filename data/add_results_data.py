    ...
    try:
        async with await psycopg.AsyncConnection.connect(DATABASE_URL) as conn:
            async with conn.cursor(row_factory=scalar_row) as cur:
                ...
                id = await cur.execute(command, criteria)
    except psycopg.errors.UniqueViolation:
        return None


def CheckPairings(event_id: int, round_number: int, p1name: str, p2name: str) -> bool:
...
def InsertStanding(event_id: int, player: Standing, submitter_id: int) -> int | None:
    conn = psycopg.connect(DATABASE_URL)
    with conn, conn.cursor() as cur:
        try:
            command = """
