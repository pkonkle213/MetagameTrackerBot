...
async def CheckPairings(event_id: int, round_number: int, p1name: str, p2name: str) -> bool:
    async with await psycopg.AsyncConnection.connect(DATABASE_URL) as conn:
...
async def InsertStanding(event_id: int, player: Standing, submitter_id: int) -> int | None:
    async with await psycopg.AsyncConnection.connect(DATABASE_URL) as conn:
        async with conn.cursor() as cur:
            try:
                command = """
