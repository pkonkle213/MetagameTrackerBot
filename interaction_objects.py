...
from tuple_conversions import Format, Game, Hub, InteractionObjects, Region, Store


def GetObjectsFromInteraction(interaction: Interaction) -> InteractionObjects:
...
def GetFormatForHub(channel_id: int, discord_id: int) -> Format | None:
    format = db.GetFormatByMap(channel_id, discord_id)
    return format


def GetFormatForStore(
    game: Game | None, channel_id: int, discord_id: int
) -> Format | None:
