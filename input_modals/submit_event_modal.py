...
class SubmitEventModal(discord.ui.Modal, title="Select Event"):
    def __init__(self, store: Store, game: Game, format: Format):
        ...
        self.data_types: list[discord.SelectOption] = [
            discord.SelectOption(label=type.name, value=str(type.value))
            for type in DataInputEnum
        ]
        ...
        self.date_input = discord.ui.Label(
            ...
            component=discord.ui.TextInput(
                placeholder="MM/DD/YYYY", default=today, required=False, max_length=10
            ),
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        self.submitted_event = SetEventInfo(
            ...
            if self.event_type.component.values
            else None,
            self.store,
            self.game,
            self.format,
        ...
        if i == 0:
            types.append(
                discord.SelectOption(
                    label=event_types[i].name,
                    value=str(event_types[i].id),
                    default=True,
                )
            )
        else:
            types.append(
                discord.SelectOption(
                    label=event_types[i].name, value=str(event_types[i].id)
                )
            )
...
def SetPastEventsOptions(
    previous_events: list[Event], default_id: int
    ...
    for i in range(len(previous_events)):
        ...
        if option.id == default_id:
            past_events.append(
                discord.SelectOption(label=label, value=value, default=True)
            )
        else:
    ...
    past_events.append(
        discord.SelectOption(
            ...
            value="0",
            default=True if default_id == 0 else False,
        )
    )
    return past_events
