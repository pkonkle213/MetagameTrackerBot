...
async def EventCheck(bot: commands.Bot):
    ...
    try:
        for channel in channels:
            ...
            archetypes = GetUnknownArchetypes(
                channel.DiscordID,
                channel.GameID,
                channel.FormatID,
                start_date,
            ...
            if not archetypes or len(archetypes) == 0:
                print(
                    f"No unknown archetypes found for {channel.DiscordID}, {channel.GameID}, {channel.FormatID}"
                )
